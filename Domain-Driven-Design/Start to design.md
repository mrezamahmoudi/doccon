...
> "Is this a thing that holds state and enforces rules?"  
> or  
> "Is this an action that coordinates multiple things?"

### Holds state + enforces rules → Domain Aggregate (class)

Example:

- `Incident` (tracks active/inactive)

### Coordinates multiple things → Application Command (class)

Example:

- `StartIncident` (calls `incident.start()` + `notification.send()` + `oncall.get()`)

### Talks to external system → Infrastructure Adapter (class)

Example:

- `IncidentNotificationService` (wraps Teams/Element/Mattermost)

### Pure data, no behavior → Value Object or DTO (dataclass)

Example:

- `OncallMember(name, role)`
...
If your diagram has a line going from the domain outward (e.g., `Incident` calling `TeamsService`), that's a violation.

The domain should not know about infrastructure.
...
Because you want to test your business logic without hitting real systems.

The adapter is the only place that imports the external library.
...
If you can describe the rule without mentioning any technology, it's domain logic.
...
For any new feature, create files in this structure:
...
- `ServiceHealth` (tracks consecutive failures)

### Domain ports

- `HealthCheckPort` (abstract — "check if service X is healthy")
- `NotificationPort` (reuse existing)

### Domain rule

- "Don't alert during active incident"
...
> The domain should be importable and testable without installing any external library.

If you import `requests` or `sqlalchemy` in a domain file, something is in the wrong place.
